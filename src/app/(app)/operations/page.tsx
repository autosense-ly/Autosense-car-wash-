...
import {
  ...
  type OperationsTranslations,
} from "@/lib/i18n/operations"
...
function getStatusLabel(
  ...
  switch (status) {
    case "waiting":
      return t.counts.waiting
    case "in_progress":
      return t.counts.inProgress
    case "ready":
      return t.counts.ready
    case "completed":
      return t.counts.completed
    case "cancelled":
...
function getNextActionLabel(
  ...
  switch (status) {
    case "waiting":
      return t.actions.startJob
    case "in_progress":
      return t.actions.markReady
    case "ready":
...
export default function OperationsPage() {
  ...
  const t = operationsTranslations[language]
  ...
  async function loadData() {
    ...
    if (jobError) {
      toast.error(
        `${t.errors.loadJobs}: ${jobError.message}`,
      )
    } else {
      setJobs(
        (jobData as unknown as Job[]) ?? [],
      )
    }
  ...
  async function advanceJob(job: Job) {
    ...
    if (error) {
      toast.error(
        `${t.errors.updateJob}: ${error.message}`,
      )
      return
    }
  ...
  async function collectPayment(
    ...
    if (error) {
      toast.error(
        `${t.errors.recordPayment}: ${error.message}`,
      )
      return
    }
    ...
    toast.success(
      `${t.payment.markedPaidVia} ${methodLabel}`,
    )
